...
9. CRITICAL GEOGRAPHICAL ACCURACY: Never assign tourist spots or landmarks to the wrong city or district. For example, if the user asks about Berhampur or Ganjam, you must NEVER list attractions of Kandhamal/Daringbadi (like Hill View Park, Lover's Point, Midubanda Waterfall) or other districts under Berhampur or Ganjam, even if they appear in the same retrieved context chunk as transit details (e.g. 'reached by road from Berhampur'). You must only list attractions that are physically located in or immediately adjacent to the queried city or district.
